            ...
            <div class="row">
                <div class="col-md-8">
                	...
                	@if(!$set_reload_amount)
	                    ...
	                    {{--
	                    <div class="shadow-box form-step step4 hide"> <!-- hide -->
	                		<ul class="row list">
	                        	<li class="step-name col-sm-10 col-xs-9">Special Offers</li>
	                        	<li class="text-right step-num col-sm-2 col-xs-3">04</li>
	                        </ul>
	                        <div class="row">
	                        	<div class="col-sm-8">
	                        		<div class="check-box">
										<label for="offer">I would like to accept this offer and buy this telephone</label>
	                       				<input type="checkbox" name="like_offer" id="like-offer" />
									</div>
									<div class="check-box">
										<label for="offer">No, thanks</label>
	                       				<input type="checkbox" name="dislike_offer" id="dislike-offer" checked />
									</div>
	                        	</div>
	                        	<div class="col-sm-4">
	                    			<div class="quantity">
	                					...
	                					<div class="col-sm-6">
	                						<div class="form-group">
		                                        <input type="number" class="form-control" value="0" name="qty-offer" id="qty-offer" value="qty-offer" disabled>
		                                    </div>
	                					</div>
	                					<div class="clearfix"></div>
	                    			</div>
	                    			<div class="text-right top-space">
	                    				<button type="button" data-next="step5" id="btn-step4" class="rounded-btn btn-next">Next</button>
	                    			</div>
	                        	</div>
	                        </div>
	                    </div>
                    ...
                    <div class="shadow-box form-step step6 hide"> <!-- hide -->
                        ...
                        <div class="top-space">
                			<button type="button" id="btn-paypal-payment" class="rounded-btn big-btn">Pay with Visa or Mastercard</button>
                		</div>
                    </div>
                </div>
                <div class="col-md-4">
		...
		$(function(){
			...
			@if(!$set_reload_amount)
				...
				$(".btn-next").click(function(){
					if($(this).data('next') == 'step3'){
					...
					else if($(this).data('next') == 'step4' && "{{Auth::guest()}}"){
						...
						$.ajax({
					        ...
					        data: {
					            ...
					            qty_micro: $('#qty-micro').val(),
					            qty_nano: $('#qty-nano').val(),
					            relaod_amount: @if(!$set_reload_amount) $('#reload-prices').val() @else "{{ $reload_amount }}" @endif,
					            reload_number: @if(!$set_reload_amount) $('#frmRelaod input[name="card_number"]').val() @else "{{ $reload_number }}" @endif
					        },
					        success: function(r){
					        	if(r.status == 200){
					        		var data = r.data;
				...
				$('#btn-step1').click(function(){
					...
					if(type == 'buy'){
						...
						$('.step3 , .step4 , .step5 , .step6').addClass('hide');
					}
					else{
						$('.buy-section').hide();
						$('.step3 , .step4 , .step5 , .step6').addClass('hide');
				...
				/**
				 * Step - 4 varifications and scripts
				 */
				$("#like-offer").change(function(event){
					if($('#dislike-offer').closest('.jcf-checkbox').hasClass('jcf-checked')){
						$("#dislike-offer").trigger('click');
					}
					else if($('#like-offer').closest('.jcf-checkbox').hasClass('jcf-checked')){
						$("#like-offer").trigger('click');
					}
					$('#qty-offer').val('1').removeAttr('disabled');
				});

				$("#dislike-offer").change(function(){
					if($('#like-offer').closest('.jcf-checkbox').hasClass('jcf-checked')){
						$("#like-offer").trigger('click');
					}
					else if($('#dislike-offer').closest('.jcf-checkbox').hasClass('jcf-checked')){
						$("#dislike-offer").trigger('click');
					}
					$('#qty-offer').val('0').attr('disabled', 'disabled');
				});
			@else
				$(".btn-next").click(function(){
					var action = "{{ route('paypal_payment') }}";
			...
			$("#btn-card-payment").click(function(){
				if($("#frmCardPayment").valid()){
					var card_expire = ($("#frmCardPayment input[name='expire']").val()).split('/');
					var action = "{{ route('card_payment') }}";
					var r_amount = @if(!$set_reload_amount) $('#reload-prices').val() @else {{ $reload_amount }} @endif;
					var r_number = @if(!$set_reload_amount) $('#frmRelaod input[name="card_number"]').val() @else {{ $reload_number }} @endif;
					$.ajax({
				        url: action,
				        type: 'POST',
				        dataType: 'json',
				        ...
				        data: {
				            _token: $('meta[name="csrf_token"]').attr('content'),
				            type: $("#card_type").html(),
				            card_holder_name: $("#frmCardPayment input[name='card_holder_name']").val(),
				            card_number: $("#frmCardPayment input[name='card_number']").val(),
				            month: card_expire[0],
				            year: card_expire[1],
				            cvv: $("#frmCardPayment input[name='cvv']").val(),
				            qty_regular: $('#qty-regular').val(),
				            qty_micro: $('#qty-micro').val(),
				            qty_nano: $('#qty-nano').val(),
				            relaod_amount: r_amount,
				        },
				        success: function(r){
				        	if(r.status == 200){
				        		...
				        		if(r_amount != ''){
					                ...
					                call_api(data, function(r){
					                    if(r.status == 200){
					                        ...
					                        data = {'api_name' : 'reload', 'card' : r_number, 'amount' : r_amount, 'orderid' : parseInt(account.orderid)+1};
					                        call_api(data, function(r){
					                            if(r.status == 200){
					                                showMessage(200, "Card successfully reload.");
					                                $.ajax({
					                                	url: "{{ route('add_card_reload_record') }}",
					                                	type: 'POST',
					                                	data: {
					                                		_token: $('meta[name="csrf_token"]').attr('content'),
					                                		card_reload: 'y',
									...
									$.ajax({
								        ...
								        success: function(r){
								        	if(r.status == 200){
												showMessage(200, "Order successfully done.");
								        	}
								        	else{
								        		showMessage(r.status, r.message);
								        	}
				        		...
				        		if(r.message != ''){
				        			var error = JSON.parse(r.message);
				        			if(error.details != undefined){
				        				showMessage(r.status, error.details[0].issue);
				        			}
				        			else{
				        				showMessage(r.status, 'Paypal service not working. Please try again after some time.')
				        			}
				        		}
				        		else{
				        			showMessage(r.status, 'Something went wrong. Please try again after some time.');
				        		}
				        	}
				        	else{
				        		showMessage(r.status, r.message);
			...
			$("#frmCardPayment input[name='card_number']").focus(function(){
