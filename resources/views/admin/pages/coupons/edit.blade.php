                ...
                <form id="frmCoupon" class="form-horizontal" role="form" method="POST" action="{{ route('admin.coupons.update', $coupon->id) }}" enctype="multipart/form-data">
                    ...
                    <div class="form-group{{ $errors->has('discount') ? ' has-error' : '' }}">
                        ...
                        <div class="col-md-4">
                            <div class="input-icon">
                                <i class="fa fa-edit"></i>
                                <input type="text" min="0" max="100"  pattern="\d*" class="form-control" name="discount" id="discount" placeholder="Enter Discount" value="{{ $coupon->discount }}">
                    ...
                    <div class="form-group{{ $errors->has('min_order_amount') ? ' has-error' : '' }}">
                        ...
                        <div class="col-md-4">
                            <div class="input-icon">
                                <i class="fa fa-edit"></i>
                                <input type="text" min="0" pattern="\d*" class="form-control" name="min_order_amount" id="min_order_amount" placeholder="Enter Amount" value="{{ $coupon->min_order_amount }}">
                    ...
                    <div class="form-group{{ $errors->has('card') ? ' has-error' : '' }}">
                        <label for="card" class="col-md-2 control-label">Select card type
                          <button type="button" data-toggle="tooltip" data-placement="top" title="Discount will be applied to selected card from given." class="tooltip-btn"><i class="fa fa-question-circle" aria-hidden="true"></i></button>
                        </label>
                        <div class="col-md-4">
                            <div class="input-icon">
                                <i class="fa fa-edit"></i>
                                <select class="form-control" name="card" id="card">
                                    ...
                                    <option value="{{$card->id}}" @if($coupon->card_type_id==$card->id) selected @endif >{{ $card->type_name }}</option>
                                    @endforeach
                                </select>
                                @if ($errors->has('card'))
                                    <span class="help-block">
